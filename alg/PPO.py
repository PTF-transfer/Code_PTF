...
class PPO:
    ...
    def build_actor_net(self, scope, trainable=True):
        with tf.variable_scope(scope):
            if self.args['use_layer_2']:
                layer_1 = tf.layers.dense(inputs=self.obs, units=self.args['n_layer_a_1'], activation=tf.nn.relu6, trainable=trainable)
                layer_2 = tf.layers.dense(inputs=layer_1, units=self.args['n_layer_a_1'], activation=tf.nn.relu6,
                                          trainable=trainable)
                if self.args['continuous_action']:
                ...
                layer_1 = tf.layers.dense(inputs=self.obs, units=self.args['n_layer_a_1'], activation=tf.nn.relu6,
                                          trainable=trainable)
                if self.args['continuous_action']:
                    mu = self.args['action_clip'] * tf.layers.dense(inputs=layer_1, units=self.n_actions,
                                                                    activation=tf.nn.tanh, trainable=trainable)
    ...
    def build_critic_net(self, scope):
        with tf.variable_scope(scope):
            if self.args['use_layer_2']:
                ...
                layer_2 = tf.layers.dense(inputs=layer_1, units=self.args['n_layer_c_1'], activation=tf.nn.relu6)
