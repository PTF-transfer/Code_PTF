...
class PPO:
    ...
    def build_actor_net(self, scope, trainable=True):
        with tf.variable_scope(scope):
            layer_1 = tf.layers.dense(inputs=self.obs, units=self.args['n_layer_a_1'], activation=tf.nn.relu6, trainable=trainable)
            if self.args['continuous_action']:
    ...
    def build_critic_net(self, scope):
        with tf.variable_scope(scope):
            layer_1 = tf.layers.dense(inputs=self.obs, units=self.args['n_layer_c_1'], activation=tf.nn.relu6)
    ...
    def build_loss(self):
        ...
        with tf.variable_scope('update_actor'):
            ...
            with tf.variable_scope('loss/clip'):
                ...
                entropyTS = tf.reduce_sum(t * otherEntroy, axis=1,
                                           keepdims=True)
                weight = 0.5 + tf.tanh(3 - self.args['c3'] * self.e) / 2
                entropyTS = entropyTS * weight * 10
    ...
    def update(self, actor, s, a, r, options, terms, epi):
        ...
        if self.args['continuous_action']:
            for _ in range(self.args['epi_train_times']):
                op, loss_clip, entropy, entropyTS, a_loss = self.sess.run(
                    [self.train_a_op, self.loss_clip, self.entropy, self.entropyTS, self.a_loss],
                    {self.obs: s, self.actions: a, self.gaes: adv, self.term: terms,
                     self.mu: mu, self.sigma: sigma, self.e: epi})
                self.logger.write_tb_log('loss_clip', loss_clip, epi)
                self.logger.write_tb_log('entropy', entropy, epi)
                self.logger.write_tb_log('entropyTS', entropyTS, epi)
                self.logger.write_tb_log('a_loss', a_loss, epi)

        else:
            for _ in range(self.args['epi_train_times']):
                op, loss_clip, entropy, entropyTS, a_loss = self.sess.run(
                    [self.train_a_op, self.loss_clip, self.entropy, self.entropyTS, self.a_loss],
                    {self.obs: s, self.actions: a, self.gaes: adv, self.term: terms, self.s_a_prob: source_actor_prob,
                     self.e: epi})
            self.logger.write_tb_log('loss_clip', loss_clip, epi)
            self.logger.write_tb_log('entropy', entropy, epi)
            self.logger.write_tb_log('entropyTS', entropyTS, epi)
        ...
        for i in range(self.args['epi_train_times']):
            op, c_loss = self.sess.run([self.train_c_op, self.c_loss], {self.obs: s, self.rewards: r})
            self.logger.write_tb_log('c_loss', c_loss, epi)
    ...
    def __init__(self, option_dim, n_features, args, sess, logger):
        ...
        with tf.name_scope('grad'):
            ...
            for i, (grad, var) in enumerate(gradients):
                if grad is not None:
                    gradients[i] = (tf.clip_by_norm(grad, args['option_clip_value']), var)
            ...
            for i, (grad, var) in enumerate(gradients_t):
                if grad is not None:
                    gradients_t[i] = (tf.clip_by_norm(grad, args['option_clip_value']), var)
    ...
    def _build_net(self, scope, s, reuse=False):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            with tf.variable_scope("option_value"):
                q_omega = tf.layers.dense(l_a, self.option_dim, None, kernel_initializer=w_init, name='omega_value')
